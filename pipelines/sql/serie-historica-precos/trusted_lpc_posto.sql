-- Trusted: LPC nivel posto — qus (4 sem) + dsan gasolina/etanol 2024-2025
-- Variaveis: {{RAW_DIR}}, {{TRUSTED_PARQUET}}

CREATE OR REPLACE TABLE trusted AS
WITH raw_qus AS (
    SELECT *, replace(filename, '\', '/') AS _filename
    FROM read_csv(
        '{{RAW_DIR}}/qus/ultimas-4-semanas-*.csv',
        filename = TRUE,
        header = TRUE,
        union_by_name = TRUE
    )
...
raw_dsan AS (
    SELECT *, replace(filename, '\', '/') AS _filename
    FROM read_csv(
        ['{{RAW_DIR}}/dsan/2024/precos-gasolina-etanol-*.csv', '{{RAW_DIR}}/dsan/2025/precos-gasolina-etanol-*.csv'],
        filename = TRUE,
        header = TRUE,
        union_by_name = TRUE
    )
