...
PRECOS_FALLBACK = REPO_ROOT / "data/trusted/serie-historica-precos/qus_gasolina_etanol.parquet"
...
def main() -> None:
    path = PRECOS if PRECOS.exists() else PRECOS_FALLBACK
    pre = pd.read_parquet(path, columns=["cnpj", "produto", "data_coleta", "valor_venda"])
    ...
    lines = [
        ...
        "",
        f"Precos: `{path.name}` · Cadastro: `revendas.parquet`",
        "",
        "| Metrica | Valor |",
        "|---------|------:|",
        f"| CNPJs distintos precos | {len(pre_c):,} |",
